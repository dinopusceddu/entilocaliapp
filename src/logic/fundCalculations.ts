...
/**
 * Calcola i totali per il Fondo Accessorio Dipendente (FAD) e restituisce il dettaglio strutturato.
 */
export const calculateFadTotals = (
  ...
  for (const [key, config] of Object.entries(strutturaFondo)) {
    ...
    const value = getFadEffectiveValueHelper(
...
export const calculateDirigenzaSubFund = (dirData: FondoDirigenzaData, hasDirigenza: boolean) => {
  ...
  const stabile = sommaRisorseStabiliDir + lim_adjustments;
  const variabile = sommaRisorseVariabiliDir;
  return { stabile, variabile, totale: stabile + variabile };
};
